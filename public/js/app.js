...
import { tasks, schedule: legacySchedule, habits, goals, progress, handleToolAction } from './features/tools.js';
...
document.getElementById('resetBtn').addEventListener('click', () => {
  ...
  resetState(); updateStreak(); showToast('Đã đặt lại dữ liệu.'); navigate('dashboard');
});
...
appView.addEventListener('click', event => {
  ...
  if(scheduleResult==='handled')return;
...
appView.addEventListener('submit', event => {
  ...
  if(scheduleResult==='handled')return;
  if(scheduleResult==='refresh'){event.preventDefault();renderCurrentRoute();return;}
...
applyTheme(getState().theme); updateStreak(); initRouter();
