...
function dueDateTime(task) {
  ...
  const value = new Date(`${task.dueDate}T${time}:00`);
...
function countdownText(task, nowDate = new Date()) {
...
export function tasks() {
  ...
  filtered.sort((a, b) => {
    const da = dueDateTime(a)?.getTime() || Number.MAX_SAFE_INTEGER;
    const db = dueDateTime(b)?.getTime() || Number.MAX_SAFE_INTEGER;
    return da - db;
  });
  ...
  return {
    ...
    render: () => `
      ...
      <div class="task-filters" role="toolbar" aria-label="Lọc nhiệm vụ">
        ${[['all', 'Tất cả'], ['today', 'Hôm nay'], ['upcoming', 'Sắp tới'], ['overdue', 'Quá hạn'], ['completed', 'Hoàn thành']].map(([key, label]) => `<button type="button" class="task-filter ${filter === key ? 'active' : ''}" data-action="task-filter" data-filter="${key}">${label}</button>`).join('')}
      </div>
      <div class="task-list">
        ${filtered.length ? filtered.map((task) => {
          ...
          const due = dueDateTime(task);
          const overdue = !task.done && Boolean(due) && due.getTime() < Date.now();
          const countdown = !task.done ? countdownText(task) : null;
          let deadlineText = `Deadline: ${dateLabel(task.dueDate)}`;
          if (span !== null && span > 0) deadlineText = `${dateLabel(task.startDate)} → ${dateLabel(task.dueDate)}`;
          if (task.dueTime) deadlineText += ` · ${timeLabel(task.dueTime)}`;
          let status = '';
          if (task.done) status = '✓ Hoàn thành';
          else if (countdown) status = `⏱ ${countdown}`;
          else if (overdue) status = remaining < 0 ? `⚠ Quá hạn ${Math.abs(remaining)} ngày` : '⚠ Đã quá deadline';
          else if (remaining === 0) status = '⚠ Deadline hôm nay';
          else if (remaining !== null) status = `Còn ${remaining} ngày`;

          return `
            <article class="task-card ${task.done ? 'is-done' : ''} ${overdue ? 'is-overdue' : ''} ${countdown ? 'is-urgent' : ''}">
              <button type="button" class="check-button" data-action="task-toggle" data-id="${task.id}" aria-label="Hoàn thành">${task.done ? '✓' : ''}</button>
              <div class="task-main">
                <div class="task-title-row"><strong>${esc(task.title)}</strong><span class="priority priority-${esc(task.priority)}">${esc(task.priority)}</span></div>
...
function openTaskModal() {
  ...
  root.insertAdjacentHTML('beforeend', `
    <div class="task-modal-backdrop" data-action="task-close-modal">
      <section class="task-modal" role="dialog" aria-modal="true" aria-labelledby="taskModalTitle">
        <form data-action="task-create" class="task-form">
          <header class="task-modal-header"><div><span class="eyebrow">NEW TASK</span><h2 id="taskModalTitle">Thêm nhiệm vụ</h2><p>Đặt khoảng thời gian, deadline và giờ hết hạn.</p></div><button type="button" class="icon-button" data-action="task-close-modal" aria-label="Đóng">×</button></header>
          <div class="task-form-grid">
            ...
            <label class="task-field"><span>Ngày bắt đầu</span><input type="date" name="startDate" value="${date}"></label>
            <label class="task-field"><span>Deadline</span><input type="date" name="dueDate" value="${date}" required></label>
            <label class="task-field"><span>Giờ deadline <small>(không bắt buộc)</small></span><input type="time" name="dueTime" value="23:59"><small class="field-hint">Trong 24 giờ cuối, ScholarOS sẽ hiển thị đếm ngược theo giờ/phút.</small></label>
            <label class="task-field"><span>Độ ưu tiên</span><select name="priority"><option value="low">Thấp</option><option value="medium" selected>Trung bình</option><option value="high">Cao</option></select></label>
          </div>
...
export function handleTaskAction(action, id, event, target) {
  ...
  if (action === 'task-close-modal') {
    ...
    if (!insideModal || event.target.closest('button[data-action="task-close-modal"]')) closeTaskModal();
    return;
  }
