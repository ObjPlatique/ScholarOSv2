...
function renderSchedule(state, calendar, selectedDate) {
  ...
  const activeId=calendar.id;
  return `<div class="schedule-module">
    <div class="schedule-tabs" role="tablist" aria-label="Các lịch">
      ${visibleCalendars(state).map(c=>`<button class="schedule-tab ${c.id===activeId?'active':''}" data-action="schedule-calendar-switch" data-id="${esc(c.id)}">${esc(c.name)}</button>`).join('')}
    ...
    <div class="schedule-hint"><span>💡</span><span>Chọn <b>+ Thêm phiên</b> để mở popup đầy đủ. Mỗi lịch có dữ liệu riêng; đổi tab sẽ chuyển sang lịch tương ứng.</span></div>
  </div>`;
}
...
function modal(mode, event={}) {
  const state=getState(); const calendars=visibleCalendars(state); const active=activeCalendar(state);
...
export function handleScheduleAction(action, id, event) {
  ...
  if(action==='schedule-delete'){if(!confirm('Xóa phiên/lịch này?'))return 'handled';if(id){const owner=calendars.find(c=>c.events.some(e=>e.id===id));if(owner){owner.events=owner.events.filter(e=>e.id!==id);calendars=syncCalendar(owner,calendars);}}else if(calendars.length>1){calendars=calendars.filter(c=>c.id!==active.id);setState({calendars,activeCalendarId:calendars[0].id,schedule:calendars[0].events});return 'refresh';}setState({calendars,schedule:active.events});closeModal();return 'refresh';}
...
function saveForm(form,id){const data=new FormData(form);const item={id:id||uid(),title:String(data.get('title')).trim(),date:String(data.get('date')),start:String(data.get('start')),duration:Number(data.get('duration'))||50,type:String(data.get('type')),focusLinked:data.get('focusLinked')==='on',notes:String(data.get('notes')||'')};let state=normalizeState();let calendars=visibleCalendars(state);let target=calendars.find(c=>c.id===String(data.get('calendarId')))||activeCalendar(state);if(id){const old=calendars.find(c=>c.events.some(e=>e.id===id));if(old)old.events=old.events.filter(e=>e.id!==id);}target.events=[...(target.events||[]),item];setState({calendars,activeCalendarId:target.id,schedule:target.events});closeModal();}

// Keep the modal inside #appView so the existing delegated action/submit
// listeners in app.js can receive its buttons and form events. Previously the
// modal was appended to document.body, outside appView, so its close/save
// controls were never handled.
