...
function modal(title, body, footer = '') {
  return `<div class="schedule-modal-backdrop" data-action="schedule-close-modal"><div class="schedule-modal" role="dialog" aria-modal="true" aria-label="${esc(title)}"><div class="schedule-modal-head"><div><span class="eyebrow">THỜI GIAN BIỂU</span><h2>${esc(title)}</h2></div><button class="icon-button" data-action="schedule-close-modal" aria-label="Đóng">×</button></div>${body}<div class="schedule-modal-footer">${footer}</div></div></div>`;
}
...
export function schedule() {
  ...
  return { title: 'Thời gian biểu', description: 'Xây dựng lịch học có cấu trúc nhưng vẫn linh hoạt.', render: () => `<div class="schedule-module">
    <div class="schedule-topbar"><div class="schedule-tabs" role="tablist" aria-label="Các lịch học">${state.calendars.map(c => `<button class="schedule-tab ${c.id === active.id ? 'active' : ''}" data-action="schedule-select-calendar" data-id="${esc(c.id)}" style="--tab-color:${esc(c.color || '#2563eb')}">${esc(c.name)}</button>`).join('')}<button class="schedule-tab-add" data-action="schedule-add-calendar" title="Tạo lịch mới">＋</button></div><div class="schedule-actions"><button class="button" data-action="schedule-edit-calendar" data-id="${esc(active.id)}">⚙ Lịch</button><button class="button primary" data-action="schedule-add">＋ Thêm phiên</button></div></div>
...
export function handleScheduleAction(action, id, event) {
  ...
  if (action === 'schedule-submit-form') { const form = event.currentTarget.closest('.schedule-modal')?.querySelector('form[data-action="schedule-save"],form[data-action="schedule-save-calendar"]'); form?.requestSubmit(); return null; }
  if (action === 'schedule-save') { const form = event.currentTarget; const data = readForm(form); if (!data.title) { form.querySelector('[name="title"]')?.focus(); return null; } const items = [...state.schedule]; const index = items.findIndex(x => x.id === id); const next = { id: id || uid('slot'), ...data }; if (index >= 0) items[index] = { ...items[index], ...next }; else items.push(next); setState({ schedule: items }); closeModal(); return 'refresh'; }
