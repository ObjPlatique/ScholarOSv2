...
function normalizeEvent(item) {
  ...
  return {
    id:item.id || uid(), title:item.title || 'Phiên học', date:item.date || isoDate(new Date()),
    start:item.start || item.time || '19:00', duration:Number(item.duration) || 50,
    type, recurrence:item.recurrence === 'weekly' ? 'weekly' : 'once',
    color:item.color || defaultColor(type), notes:item.notes || '', focusLinked:Boolean(item.focusLinked)
  };
}
...
function normalizeState() {
  ...
  if (state.calendars?.length) {
    state.calendars.forEach(calendar => { calendar.events = (calendar.events || []).map(normalizeEvent); });
    return state;
  }
...
function renderSchedule(state, calendar, selectedDate) {
  const days=calendarRange(selectedDate);
  const events=calendar.events || [];
  return `<div class="schedule-module">
    <div class="schedule-tabs" role="tablist" aria-label="Các lịch">
      ${visibleCalendars(state).map(c=>`<button class="schedule-tab ${c.id===calendar.id?'active':''}" data-action="schedule-calendar-switch" data-id="${esc(c.id)}">${esc(c.name)}</button>`).join('')}
      <button class="schedule-tab-add" data-action="schedule-calendar-add" aria-label="Tạo lịch mới">+</button>
    </div>
    <div class="schedule-module-toolbar">
      <div><h2 class="section-title">${esc(calendar.name)}</h2><p class="muted">${events.length} phiên · Tuần ${days[0].toLocaleDateString('vi-VN',{day:'numeric',month:'numeric'})} – ${days[6].toLocaleDateString('vi-VN',{day:'numeric',month:'numeric'})}</p></div>
      <div class="schedule-toolbar-actions"><div class="schedule-side-tools"><button class="button" data-action="schedule-calendar-rename">Đổi tên</button><button class="button" data-action="schedule-calendar-delete">Xóa lịch</button></div><button class="button primary" data-action="schedule-add">+ Thêm phiên</button></div>
    </div>
    <div class="schedule-calendar-card">
      <div class="schedule-calendar-toolbar"><div class="schedule-calendar-nav"><button class="button" data-action="schedule-prev">←</button><button class="button" data-action="schedule-today">Hôm nay</button><button class="button" data-action="schedule-next">→</button></div><strong>${monthLabel(days[3])}</strong><span class="schedule-view-label">Tuần</span></div>
    ...
    <div class="schedule-hint"><span>💡</span><span><b>+ Thêm phiên</b> sẽ thêm trực tiếp vào lịch đang chọn. Bạn có thể chọn <b>một lần</b> hoặc <b>hàng tuần</b>, đồng thời đặt màu riêng cho từng phiên.</span></div>
  </div>`;
}
...
function modal(mode, event={}) {
  ...
  const value={
    title:event.title||'',date:event.date||isoDate(new Date()),start:event.start||'19:00',duration:event.duration||50,
    type:event.type||'study',recurrence:event.recurrence||'once',color:event.color||defaultColor(event.type),
    focusLinked:Boolean(event.focusLinked),notes:event.notes||''
  };
  const colorOptions=['#2563eb','#8b5cf6','#10b981','#f59e0b','#ef4444','#ec4899'];
  return `<div class="schedule-modal-backdrop" data-schedule-modal data-action="schedule-modal-close"><div class="schedule-modal" role="dialog" aria-modal="true" aria-labelledby="scheduleModalTitle" onclick="event.stopPropagation()"><div class="schedule-modal-head"><div><p class="eyebrow">TIME SCHEDULE</p><h2 id="scheduleModalTitle">${mode==='edit'?'Chỉnh sửa phiên':'Thêm phiên học'}</h2><p class="schedule-modal-subtitle">Đang thêm vào <strong>${esc(active.name)}</strong></p></div><button type="button" class="icon-button" data-action="schedule-modal-close" aria-label="Đóng">×</button></div><form class="schedule-form" data-action="schedule-save" data-id="${esc(event.id||'')}">
    <label>Tên phiên<input name="title" required maxlength="100" value="${esc(value.title)}" placeholder="Ví dụ: Toán — Hàm số"></label>
    <div class="schedule-form-grid"><label>Ngày<input name="date" type="date" required value="${value.date}"><small class="form-help">Với phiên hàng tuần, ngày này xác định thứ trong tuần.</small></label><label>Giờ bắt đầu<input name="start" type="time" required value="${value.start}"></label></div>
    <div class="schedule-form-grid"><label>Thời lượng<select name="duration"><option value="25" ${value.duration==25?'selected':''}>25 phút</option><option value="50" ${value.duration==50?'selected':''}>50 phút</option><option value="90" ${value.duration==90?'selected':''}>90 phút</option><option value="120" ${value.duration==120?'selected':''}>120 phút</option></select></label><label>Loại nội dung<select name="type"><option value="study" ${value.type==='study'?'selected':''}>Học tập</option><option value="review" ${value.type==='review'?'selected':''}>Ôn tập</option><option value="test" ${value.type==='test'?'selected':''}>Kiểm tra</option><option value="personal" ${value.type==='personal'?'selected':''}>Cá nhân</option></select></label></div>
    <fieldset class="schedule-choice-group"><legend>Tần suất</legend><label class="schedule-choice ${value.recurrence==='once'?'selected':''}"><input type="radio" name="recurrence" value="once" ${value.recurrence==='once'?'checked':''}><span><strong>Một lần</strong><small>Chỉ xuất hiện vào ngày đã chọn</small></span></label><label class="schedule-choice ${value.recurrence==='weekly'?'selected':''}"><input type="radio" name="recurrence" value="weekly" ${value.recurrence==='weekly'?'checked':''}><span><strong>Hàng tuần</strong><small>Lặp lại mỗi tuần vào cùng thứ</small></span></label></fieldset>
    <div class="schedule-color-field"><span class="schedule-field-label">Màu phiên</span><div class="schedule-color-options">${colorOptions.map(color=>`<label class="schedule-color-option" style="--choice-color:${color}"><input type="radio" name="colorPreset" value="${color}" ${value.color===color?'checked':''}><span title="Chọn màu"></span></label>`).join('')}<label class="schedule-color-custom"><input type="color" name="color" value="${esc(value.color)}" aria-label="Màu tùy chỉnh"></label></div></div>
    <label class="schedule-checkbox"><input type="checkbox" name="focusLinked" ${value.focusLinked?'checked':''}> Liên kết với Focus</label>
    <label>Ghi chú<textarea name="notes" rows="3" maxlength="500" placeholder="Ghi chú cho phiên học...">${esc(value.notes)}</textarea></label>
  </form><div class="schedule-modal-actions"><button type="button" class="button" data-action="schedule-modal-close">Hủy</button><button type="button" class="button primary" data-action="schedule-save-submit" data-id="${esc(event.id||'')}">${mode==='edit'?'Lưu thay đổi':'Thêm phiên'}</button>${mode==='edit'?`<button type="button" class="button danger-outline" data-action="schedule-delete" data-id="${esc(event.id)}">Xóa phiên</button>`:''}</div></div></div>`;
}
...
export function handleScheduleAction(action, id, event) {
  ...
  if(action==='schedule-calendar-switch'){setState({activeCalendarId:id,schedule:(calendars.find(c=>c.id===id)||active).events});return 'refresh';}
...
function saveForm(form,id){
  ...
  const type=String(data.get('type')||'study');
  const preset=String(data.get('colorPreset')||'');
  const custom=String(data.get('color')||'');
  const item={
    id:id||uid(), title:String(data.get('title')).trim(), date:String(data.get('date')), start:String(data.get('start')),
    duration:Number(data.get('duration'))||50, type, recurrence:data.get('recurrence')==='weekly'?'weekly':'once',
    color:preset || custom || defaultColor(type), focusLinked:data.get('focusLinked')==='on', notes:String(data.get('notes')||'')
  };
  let state=normalizeState(); let calendars=visibleCalendars(state); const target=activeCalendar(state);
...
function openModal(html){closeModal();const host=document.getElementById('appView');if(host)host.insertAdjacentHTML('beforeend',html);}
