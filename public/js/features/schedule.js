...
function normalizeEvent(item) {
  ...
  return { id:item.id || uid(), title:item.title || 'Phiên học', date:item.date || isoDate(new Date()), start:item.start || item.time || '19:00', duration:Number(item.duration) || 50, type, recurrence:item.recurrence === 'weekly' ? 'weekly' : 'once', color:item.color || defaultColor(type), notes:item.notes || '', focusLinked:Boolean(item.focusLinked) };
}
...
function modal(mode, event={}) {
  ...
  const value={title:event.title||'',date:event.date||isoDate(new Date()),start:event.start||'19:00',duration:event.duration||50,type:event.type||'study',recurrence:event.recurrence||'once',color:event.color||defaultColor(event.type),focusLinked:Boolean(event.focusLinked),notes:event.notes||''};
...
function saveForm(form,id){
  ...
  const item={id:id||uid(),title:String(data.get('title')||'').trim(),date:String(data.get('date')||isoDate(new Date())),start:String(data.get('start')||'19:00'),duration:Number(data.get('duration'))||50,type,recurrence:data.get('recurrence')==='weekly'?'weekly':'once',color:preset||custom||defaultColor(type),focusLinked:data.get('focusLinked')==='on',notes:String(data.get('notes')||'')};
