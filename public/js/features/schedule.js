...
export function schedule() {
  ...
  return {
    ...
    description:'Quản lý nhiều lịch học trong một giao diện trực quan.',
    render: () => renderSchedule(getState(), calendar, parseDate(selectedDate))
  };
}
...
function renderDay(day, events) {
  ...
  return `<div class="schedule-day-column">${Array.from({length:15},()=>'<div class="schedule-hour-line"></div>').join('')}${events.filter(e=>e.date===date).map(e=>{
    const top=Math.max(0,minutes(e.start)-420); const height=Math.max(42,Number(e.duration||50)*64/60-4);
    return `<button class="schedule-event schedule-event-${esc(e.type||'study')}" style="top:${top+3}px;height:${height}px" data-action="schedule-edit" data-id="${esc(e.id)}"><strong>${esc(e.title)}</strong><span>${esc(e.start)} · ${Number(e.duration||50)} phút</span>${e.focusLinked?'<span class="schedule-event-focus">◷ Focus</span>':''}</button>`;
  }).join('')}</div>`;
}
