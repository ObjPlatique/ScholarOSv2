...
export function handleToolAction(action, id, event) {
  const s=getState(); let changed=true;
  ...
  if(action==='focus-toggle'){
    const f=s.focus||{minutes:25,running:false,startedAt:null,secondsLeft:1500};
    if(f.running){
      const elapsed = f.startedAt ? Math.floor((Date.now() - f.startedAt) / 1000) : 0;
      const remaining = Math.max(0, f.secondsLeft - elapsed);
      setState({focus:{...f,running:false,startedAt:null,secondsLeft:remaining}});
    } else {
      const remaining = Math.max(0, Number(f.secondsLeft) || (Number(f.minutes)||25)*60);
      setState({focus:{...f,running:true,startedAt:Date.now(),secondsLeft:remaining}});
    }
    return 'refresh';
  }
