...
(() => {
  ...
  function getTodayColumn() {
    ...
    const columns = [...app.querySelectorAll('.schedule-day-column')];
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      ...
      .schedule-current-time-indicator::before{content:"";position:absolute;left:0;right:0;top:0;height:2px;background:var(--primary,#2563eb);box-shadow:0 0 0 1px color-mix(in srgb,var(--primary,#2563eb) 12%,transparent);}
  ...
  function update() {
    ...
    if (hourLines.length > 1) {
      const a = hourLines[0].getBoundingClientRect();
      const b = hourLines[1].getBoundingClientRect();
      hourHeight = Math.max(1, b.top - a.top);
    }
    const top = (minutes - START_MINUTE) / 60 * hourHeight;
